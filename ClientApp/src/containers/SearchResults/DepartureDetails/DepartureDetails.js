...
const departureDetails = props => (
	<div className="list-group mt-0 mb-2">
		<div className="list-group-item bg-light d-flex align-items-center h5 text-dark">
			...
			<span className="font-weight-bold">{props.intermediateStops.StartStation.Name}</span>
		</div>
		{props.intermediateStops.MiddleStations.map((stop, index) => (
			<a
				...
				onClick={e => {
					e.preventDefault();
					props.history.push(`/${encodeURIComponent(stop.Name.replace(/\//g, '_'))}/${stop.SiteId}`);
				}}
			>
		...
		<a
			...
			onClick={e => {
				e.preventDefault();
				props.history.push(
					`/${encodeURIComponent(props.intermediateStops.EndStation.Name.replace(/\//g, '_'))}/${
		...
		>
			<i className="fa fa-circle-o mr-3 fa-2x text-info" />
			<span className="font-weight-bold">{props.intermediateStops.EndStation.Name}</span>
		</a>
	</div>
);
